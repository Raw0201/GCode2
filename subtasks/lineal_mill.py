...
from subtasks.generators.lineal_mill_gen import lineal_mill_gen
from interfaces.ui_lineal_mill import Ui_frm_lineal_mill


class Lineal_mill(Subtask, Ui_frm_lineal_mill):
    def __init__(self, main_window):
        super().__init__()
        self.window = main_window
        self.task = subtasks.tasks_list["Lineal_mill"]["Description"]
        self.image = "lineal_mill.png"

        self.btn_switch.clicked.connect(
            lambda: self.change_to(self.window, "Radial_mill")
        )

        self.cbx_mov.addItems(lineal_moves_list)
        self.cbx_sys.addItems(coordinates_systems)
        self.cbx_com.addItems(tool_compensations)
    ...
    def validator(self, data: dict):
        ...
        if data["Xin"] == "" and data["Yin"] == "" and data["Zin"] == "":
    ...
    def packer(self, data: dict):
        ...
        collect_data(self.window, "Lineal_mill")
    ...
    def generator(self, machine: str, data: dict) -> list:
        ...
        return lineal_mill_gen(machine, data)
    ...
    def modifier(self, data: dict):
        ...
        self.cbx_com.setCurrentText(str(com))
