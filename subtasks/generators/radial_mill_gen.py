...
def gen_b12(data: list) -> list:
    ...
    xin, yin, zin, fed, xcn, ycn, zcn, rot, sys, blk = data.values()
    blank_space = fspace()
    blk = "/" if blk else ""
...
def gen_a16(data: list) -> list:
    ...
    lines1 = [f"{blk}{rot}{xin}{yin}{zin}{rad}{fed}"]
    lines2 = [blank_space]
...
def gen_mazak(data: list) -> list:
    ...
    zin = "" if zin == "" else f"Z{fnum4(zin)}"
    fed = "" if fed == "" else f"F{ffed(fed)}"
    rot = "" if rot == "" else mill_g_codes[f"GIRO {rot}"]
    ...
    lines1 = [f"{blk}{sys}{rot}{xin}{yin}{zin}{rad}{fed}"]
    lines2 = [blank_space]
